...
test('covers sync error details for an unknown operation on published aneety.com', async ({ page }) => {
  ...
  const syncPanel = mobile.getByRole('region', { name: 'Fila de sincronização' });
  await expect(syncPanel.getByText('delete_everything')).toBeVisible();
  await expect(syncPanel.getByText('Pedido: 1008')).toBeVisible();
  await expect(syncPanel.getByText('Tentativas: 0')).toBeVisible();
...
test('covers sync error details for an invalid checkpoint payload on published aneety.com', async ({ page }) => {
  ...
  await page.evaluate(async () => {
    await new Promise<void>((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        store.clear();
        store.put({
          id: 'sync_invalid_checkpoint_payload_e2e',
          operation: 'update_checkpoint',
          orderId: '1008',
          payload: { input: { completed: true, actor: 'Playwright sem checkpointKey' } },
          createdAt: new Date().toISOString(),
          attempts: 0
        });
        tx.oncomplete = () => {
  ...
  await syncPanel.getByRole('button', { name: 'Sincronizar agora' }).click();
  await expect(page.getByText('Sincronização concluída: 0 enviados, 1 falhas.')).toBeVisible();
  await expect(syncPanel.getByText('Erro: Invalid checkpoint payload: checkpointKey is required')).toBeVisible();
  await expect(syncPanel.getByText('Tentativas: 1')).toBeVisible();
});
...
test('covers sync error details for an invalid attachment payload on published aneety.com', async ({ page }) => {
  ...
  await page.evaluate(async () => {
    await new Promise<void>((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        store.clear();
        store.put({
          id: 'sync_invalid_attachment_payload_e2e',
          operation: 'upload_attachment',
          orderId: '1008',
          payload: {},
          createdAt: new Date().toISOString(),
  ...
  await syncPanel.getByRole('button', { name: 'Sincronizar agora' }).click();
  await expect(page.getByText('Sincronização concluída: 0 enviados, 1 falhas.')).toBeVisible();
  await expect(syncPanel.getByText('Erro: Invalid attachment payload: attachmentId is required')).toBeVisible();
  await expect(syncPanel.getByText('Tentativas: 1')).toBeVisible();
});
  ...
  await page.evaluate(async () => {
    await new Promise<void>((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        store.clear();
        store.put({
          id: 'sync_invalid_payment_payload_e2e',
          operation: 'create_payment_intent',
          orderId: '1008',
          payload: {},
          createdAt: new Date().toISOString(),
