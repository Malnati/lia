...
4. Se um item permanecer pendente, corrija a origem: criação de pedido exige cliente, telefone e endereço; checkpoints exigem pedido existente e `checkpointKey`; anexos exigem pedido existente e `attachmentId`; pagamento exige pedido existente; operações desconhecidas indicam fila local corrompida e devem ser removidas ou recriadas pelo fluxo correto.
